...
class Solution:
    # THeoretically time O(m * n) but actually performs worse than the O(m * n * (m + n)) string concatenation version
    ...
    def shortestCommonSupersequence(self, str1: str, str2: str) -> str:
        ...
        R = len(longer)
        C = len(shorter)
        prev_dp = [shorter[:c] for c in range(C + 1)]
        ...
        for r in range(1, R + 1):
            ...
            for c in range(1, C + 1):
                ...
                if lch == sch:
                    dp[c] = prev_dp[c - 1] + lch
                else:
                    use_short = dp[c - 1] + sch
                    use_long = prev_dp[c] + lch
                    dp[c] = use_short if len(use_short) < len(use_long) else use_long
